...
* *The knowledge base is a SQlite database with two tables: `entities` and `relationships`. An entity describes a "thing", such as a web search result or an user. A relationship describes the connenctions between entities, such as a friendship between users.
