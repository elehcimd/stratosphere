...
3. Create a new module in `/shared/src/stratosphere/extractors` that defines a function `extract(rows: List[stratosphere.stoerage.models.Flow])`. A symbolic link to `/shared/` can be found in the root directory of JupyterLab and you can use it to add the new module. Implement the `extract` function s.t. it processes all input flows, inserting them in the knowledge base. You can use `extractor_google_search.py` as example. Recommendations:
