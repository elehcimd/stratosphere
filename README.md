...
* Entities are uniquely identified by UUIDs that are generated by hashing their unique string identifiers. The definition of the unique string identifier is not fixed and it depends on the scraper. For example, the entity of an user with ID `123` on social network `xyz.com` could be uniquely identified by the string `xyz.com-123`. Its UUID is defined by the 32-character hexadecimal string obtained from `UUID(hex=MD5("xyz.com-123"))`.
* Relationships are uniquely identified by the concatenation of their UUIDs. For example, the relationship between entities with UUIDs `123..` and `124..` is defined as `UUID(hex=MD5("123..-124.."))`.
