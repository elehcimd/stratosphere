...
* *The knowledge base is a SQlite database with two tables: `entities` and `relationships`.
...
* *Entities are uniquely identified by UUIDs that are generated by hashing their unique string identifier.
For example, the entity of an user with ID `123` on social network `xyz.com` has its UUID defined by the 32-character hexadecimal string obtained from `UUID(hex=MD5("xyz.com-123"))`. Relationships are uniquely identified by the concatenation of their UUIDs. For example, the relationship between entities with UUIDs `123..` and `124..` is defined as `UUID(hex=MD5("123..-124.."))`
